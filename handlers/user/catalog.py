...
async def category_callback_handler(query: CallbackQuery, callback_data: dict):
    products = db.fetchall(
        "SELECT * FROM products product WHERE product.tag = (SELECT title FROM categories WHERE idx=?) AND product.idx NOT IN (SELECT idx FROM cart WHERE cid = ?)",
...
async def add_product_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await query.answer("Товар был добавлен в корзину!")
